...
def search_documents_tool(question: str) -> str:
    """Search the uploaded PDF documents for information relevant to a question.

    Use when the user asks something that may be answered by a document they
    have uploaded — e.g. a schedule, syllabus, manual, or report.

    Args:
        question: The user's question or topic to search for.
            Examples: "When is the next lecture?", "What are the exam dates?"

    Returns:
        Relevant excerpts from the documents, or a message if nothing was found.
    """
    result = _rag.search(question)
    if not result:
